...
- `assets/css/poetry-refinements.css`：首页标题、作者自选推荐与随机阅读板块的细化样式。
...
“从这几首开始”固定展示 `featured_titles` 中的四首作者自选作品。“随机翻一首诗”独立从全部作品中抽取，不受目录筛选影响；同一轮不重复，点击“阅读全文”进入独立阅读页。目录旁“随机翻诗”统一跳转到该板块。
...
音乐配置在 _data/poetry_music.yml。修改曲名、作曲者、音频路径和来源即可换曲；enabled 改为 false 可关闭整个播放器。音频保存在 assets/audio/，录音许可记录在该目录的 CREDITS.md。替换音频时同步更新来源和许可。

播放器默认暂停，点击后播放并循环；展开右侧设置可调节进度和音量。当前浏览会话记住位置和音量，进入另一首诗仍需点击后继续。打印时隐藏，学术页面不加载播放器。
